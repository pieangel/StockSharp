...
class SignalMasterRestClient : BaseLogReceiver
{
	...
	public SignalMasterRestClient(SecureString key, SecureString secret)
	{
		_key = key;
	...
	public override string Name => nameof(SignalMaster) + "_" + nameof(HttpClient);
		...
		where T : class
	{
		...
		SignalMasterRestResponse<T> restResponse = Parse<SignalMasterRestResponse<T>>(response);
	...
	private Task<dynamic> ProcessSignedRequest(Method method, string endpoint, string subaccountName, string jsonBody, CancellationToken cancellationToken)
	{
		...
		request.AddHeader("SignalMaster-KEY", _key.UnSecure());
		request.AddHeader("SignalMaster-SIGN", sign);
		request.AddHeader("SignalMaster-TS", nonce.ToString());

		if (!subaccountName.IsEmpty())
		{
			request.AddHeader("SignalMaster-SUBACCOUNT", subaccountName.EncodeUrl());
		}
	...
	private async Task<T1> ProcessSignedRequest<T, T1>(Method method, string endpoint, string subaccountName, string jsonBody, CancellationToken cancellationToken)
		where T : class
		where T1 : SignalMasterRestResponse<T>
	{
		dynamic response = await ProcessSignedRequest(method, endpoint, subaccountName, jsonBody, cancellationToken);
