...
{
    public partial class StockSharpChartView : UserControl, ICandleBuilderSubscription
    {
        ...
        private readonly CachedSynchronizedDictionary<IChartIndicatorElement, StockSharp.Algo.Indicators.IIndicator> _indicators = new();
        ...
        private readonly System.Threading.Timer _dataTimer;
        ...
        public StockSharpChartView()
        {
            ...
            try
            {
                ...
                ConfigManager.RegisterService<ICandlePatternProvider>(
                    new InMemoryCandlePatternProvider(CandlePatternRegistry.All));
        ...
        private void OnLoaded(object sender, RoutedEventArgs routedEventArgs)
        {
            ...
            {
                MessageBox.Show(
                    $"RegisterOrder: sec={order.Security.Id}, {order.Side} {order.Volume}@{order.Price}");
            };
        ...
        private void Chart_OnSubscribeIndicatorElement(IChartIndicatorElement element, CandleSeries series,
            StockSharp.Algo.Indicators.IIndicator indicator)
        {
        ...
        private void RefreshCharts()
        {
            ...
            this.GuiSync(() =>
            {
				...
				//Chart.AddElement(_areaComb, _candleElement);
			});
        }
            ...
            Task.Factory.StartNew(() =>
                {
                    ...
                    if (isBuild)
                    {
                        foreach (var tick in storage
                                     .GetTickMessageStorage(secId, new LocalMarketDataDrive(path), format).Load())
                        {
                    ...
                    {
                        foreach (var candleMsg in storage.GetCandleMessageStorage(msgType, secId, dt.Arg,
                                     new LocalMarketDataDrive(path), format).Load())
                        {
                            if (token.IsCancellationRequested)
                                break;
        ...
        private static DrawingColor GetRandomColor() => DrawingColor.FromArgb(255, (byte)RandomGen.GetInt(0, 255),
            (byte)RandomGen.GetInt(0, 255), (byte)RandomGen.GetInt(0, 255));
        ...
        private void DrawChartElements()
        {
            ...
            catch (Exception ex)
            {
                ...
                // You can log or display the exception message
                Console.WriteLine($"Error in DrawChartElements: {ex.Message}");
                // Optionally, log additional details or rethrow the exception if needed
                // throw;
            }
        }
        ...
        private void CustomColors_Changed(object sender, RoutedEventArgs e)
        {
            ...
            {
                _candleElement.Colorer = (dto, isUpCandle, isLastCandle) =>
                    dto.Hour % 2 != 0 ? null : (isUpCandle ? DrawingColor.Chartreuse : DrawingColor.Aqua);
                _indicators.Keys.ForEach(el =>
                    el.Colorer = c => ((DateTimeOffset)c).Hour % 2 != 0 ? null : DrawingColor.Magenta);
            }
            else
            {
        ...
        private void ModifyAnnotation(bool isNew)
        {
            ...
            {
                var b = new SolidColorBrush(Color.FromRgb((byte)RandomGen.GetInt(0, 255),
                    (byte)RandomGen.GetInt(0, 255), (byte)RandomGen.GetInt(0, 255)));
                b.Freeze();
                return b;
            }
            ...
            var mode = RandomGen.GetDouble() > 0.5
        ...
        private class TestMarketSubscriptionProvider : ISubscriptionProvider
        {
            ...
            event Action<Subscription, object> ISubscriptionProvider.SubscriptionReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, IOrderBookMessage> ISubscriptionProvider.OrderBookReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, ITickTradeMessage> ISubscriptionProvider.TickTradeReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, IOrderLogMessage> ISubscriptionProvider.OrderLogReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, Security> ISubscriptionProvider.SecurityReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, ExchangeBoard> ISubscriptionProvider.BoardReceived
            {
                add { }
                remove { }
            }
            ...
            [Obsolete]
            event Action<Subscription, MarketDepth> ISubscriptionProvider.MarketDepthReceived
            {
                add { }
                remove { }
            }
            ...
            [Obsolete]
            event Action<Subscription, OrderLogItem> ISubscriptionProvider.OrderLogItemReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, News> ISubscriptionProvider.NewsReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, ICandleMessage> ISubscriptionProvider.CandleReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, MyTrade> ISubscriptionProvider.OwnTradeReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, StockSharp.BusinessEntities.Order> ISubscriptionProvider.OrderReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, OrderFail> ISubscriptionProvider.OrderRegisterFailReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, OrderFail> ISubscriptionProvider.OrderCancelFailReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, OrderFail> ISubscriptionProvider.OrderEditFailReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, Portfolio> ISubscriptionProvider.PortfolioReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription, StockSharp.BusinessEntities.Position> ISubscriptionProvider.PositionReceived
            {
                add { }
                remove { }
            }

            event Action<Subscription> ISubscriptionProvider.SubscriptionOnline
            {
                add { }
                remove { }
            }

            event Action<Subscription> ISubscriptionProvider.SubscriptionStarted
            {
                add { }
                remove { }
            }

            event Action<Subscription, Exception> ISubscriptionProvider.SubscriptionStopped
            {
                add { }
                remove { }
            }

            event Action<Subscription, Exception, bool> ISubscriptionProvider.SubscriptionFailed
            {
                add { }
                remove { }
            }
