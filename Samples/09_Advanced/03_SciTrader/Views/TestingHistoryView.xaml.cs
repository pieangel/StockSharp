...
using SciTrader.Strategies;

namespace SciTrader.Views
{
    /// <summary>
    /// Interaction logic for TestingHistoryView.xaml
    /// </summary>
    public partial class TestingHistoryView : UserControl
    {
		// emulation settings
		...
		private void StartBtnClick(object sender, RoutedEventArgs e)
		{
			...
			if (HistoryPath.Folder.IsEmpty() || !Directory.Exists(HistoryPath.Folder))
			{
				...
				MessageBox.Show(LocalizedStrings.WrongPath, "Message", MessageBoxButton.OK, MessageBoxImage.Information);
				return;
			}

			if (_connectors.Any(t => t.State != ChannelStates.Stopped))
			{
				MessageBox.Show(LocalizedStrings.AlreadyStarted, "Message", MessageBoxButton.OK, MessageBoxImage.Information);
				return;
			}
					...
					if (state == ChannelStates.Stopped)
					{
						...
						this.GuiAsync(() =>
						{
							if (connector.IsFinished)
							{
								...
								MessageBox.Show(LocalizedStrings.CompletedIn.Put(DateTime.Now - _startEmulationTime), title, MessageBoxButton.OK, MessageBoxImage.Information);
							}
							else
								MessageBox.Show(LocalizedStrings.Cancelled, title, MessageBoxButton.OK, MessageBoxImage.Information);
						});
					}
