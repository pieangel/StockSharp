...
namespace SciTrader.Model
{
    public class Product
    {
        ...
        public StockSymbol AddSymbol(StockSymbol item)
        {
        ...
        public StockSymbol GetRecentMonthItem()
        {
            return _symbolList.FirstOrDefault();
        }

        public StockSymbol GetNextMonthItem()
        {
            return _symbolList.ElementAtOrDefault(1);
        }
        ...
        public List<StockSymbol> GetRecentMonthItemOnOption()
        {
            var ym = GetRecentYearMonth();
            return ym?.GetSymbolList() ?? new List<StockSymbol>();
        }

        public YearMonth AddYearMonth(string yearMonthName)
        {
			if (!_yearMonthMap.TryGetValue(yearMonthName, out var ym))
			{
				ym = new YearMonth();
				_yearMonthMap[yearMonthName] = ym;
			}

            return ym;
		}

        public void AddToYearMonth(int market, string itemCode, StockSymbol item)
        {
            if (item == null)
                return;
        ...
        public void AddToYearMonth(string itemCode, string name, StockSymbol item)
        {
            if (item == null)
                return;
