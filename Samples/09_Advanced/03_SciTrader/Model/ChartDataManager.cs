...
namespace SciTrader.Model
{
    ...
    public class ChartDataManager
    {
        ...
        public void requestChartData()
        {
            try
            {
                SymbolManager itemManager = SymbolManager.Instance;
                var item = itemManager.GetFirstFavoriteSymbolByProductCode();
                if (item != null)
                {
                    var requestData = new ChartDataRequest(item.SymbolCode, "MIN", 1, 0, 1500);
